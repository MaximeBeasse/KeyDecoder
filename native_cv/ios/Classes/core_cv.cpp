...
cv::Point2f cvPointFromCoordinate(struct Coordinate* c) {
	return cv::Point2f((float)c->x, (float)c->y);
}

std::vector<cv::Point2f> nativeInter2Vect(struct NativeIntersection* points) {
...
int homography_warp(struct NativeIntersection* points, char *src, char *dst) {
